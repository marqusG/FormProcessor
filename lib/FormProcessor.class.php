...
JSC;
    ...
    */
    ...
    private function buildFileWrapper($fileName, $field, $counter)
    {
        ...
        $ext = end($parts);
        $fileWrapperMarkup = '';
        $picExt = isset($this->config['general']['valid_extension']['pictures']) ? $this->config['general']['valid_extension']['pictures'] : array('jpg', 'png', 'gif');
        $docExt = isset($this->config['general']['valid_extension']['documents']) ? $this->config['general']['valid_extension']['documents'] : array('txt', 'doc', 'odf', 'pdf');
...
FRM;
    ...
    public function processFiles($fileType, $default = null)
    {
        ...
        for ($i = 0; $i < count($_FILES["$fileType"]['name']); ++$i) {
            ...
            if (($_FILES["$fileType"]['size'][$i] <= $maxFileSize) && in_array($fileExtension, $validExtensions["$fileType"])) {
                if (move_uploaded_file($_FILES["$fileType"]['tmp_name'][$i], $this->root.$fileType.DIRECTORY_SEPARATOR.$name)) {
                    if ($fileType == 'pictures') {
                        $img = @imagecreatefromstring(@file_get_contents($this->root.$fileType.DIRECTORY_SEPARATOR.$name));
        ...
        if (count($files) > 0) {
            ...
            if ($this->editMode) {
                $existingFiles = $this->model->get_files($fileType, $this->itemId);
    ...
    */
    ...
    {
        $langFields = $this->config['translation']['translation_fields'][$this->tableName];
        $fieldsArray = array();
        $originalTextArray = array();
        foreach ($data as $key => $value) {
            if (array_key_exists($key, $langFields)) {
                array_push($fieldsArray, $key);
                array_push($originalTextArray, $value);
            }
        }

        $fields = implode(';', $fieldsArray);
        $originalText = implode(';', $originalTextArray);
        // $identifier = $this->CI->config->item('identifiers')[$this->table];
        //non metto identifier ma dovrò ricavare
        $this->model->insertNewItemToTranslate($this->table, $this->config['identifiers'][$this->tableName], $fields, $originalText);
    }
    ...
    */
    public function deletePicture($imageName, $itemId)
    {
        ...
        $pictures = $this->model->get_files('pictures', $itemId);
    ...
    */
    public function deleteDocument($docName, $itemId)
    {
        ...
        $documents = $this->model->get_files('documents', $itemId);
